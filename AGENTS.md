...
- Automated gates: `pnpm verify` (typecheck) and `pnpm test:core`（26 个测试文件，glob 覆盖 `tests/` 全部）. `pnpm build` additionally exercises the custom client bundler. Use `git diff --check` for whitespace hygiene.
