...
- Key layout（注释版仓库树；`(不入库)` = gitignore，外部 clone 不可见）:
  ...
  │  └─ probes/              ← 12 个回归锚点（builtin-only，可单跑）
  ├─ tests/                  ← 13 个 .test.ts（node --test，type-stripping 直跑）
  ├─ docs/
  │  ├─ specs/               ← 6 篇权威设计文档（入库）
...
- **CDP 探针环境参数（Termux 本机实测）**：`cdp-swipe-failures.mjs` 的 `DSH_PROBE_URL` 缺省曾误指调试端口 3457，已改为与主探针一致的 `http://127.0.0.1:3080/`（旧版不带 env 直接跑会连到没人监听的端口，报「插件 frame 未就绪」——现象像回归/环境坏了，其实是 URL 错）。主探针需要 `DSH_PROBE_SESSION_ID`（取 `~/.dsh/sessions/<项目目录>/` 最新 `session-*`；本机 3080 的 cwd 是 `~`，对应 `--data-data-com.termux-files-home--/`），Termux 上 `DSH_PROBE_CHROME=chromium-browser` 必须显式传（缺省 `chromium` 会 spawn ENOENT）。另外连续跑探针会在设备上泄漏 headless chromium 进程（实测 31 个残留、load 7.0，拖垮后续所有探针 boot 甚至误报超时）——排查前先 `pgrep -c chrom` 清点；`pkill -f` 的 pattern 会匹配自身命令行把当前 shell 杀掉，用不含自匹配的 pattern 或从另一个会话清理。 **认证实例与 phase 两个坑（2026-09-14）**：①`document.body.dataset.dshPhase` **不存在**（`grep -r dshPhase dsh-web-frontend/dist` 零命中）——旧脚本照抄它只会超时（靠 for 循环跑满不抛错才侥幸过关）；活性标记是**元素属性** `[data-phase="active"]`（会话根 `wSkVaW_root`，实测 boot 后约 3.3s 出现），插件自己的 `[data-mobile-nav="frame"]` 更早（约 0.7s）——要「插件已武装」就用后者，要「宿主会话已就绪」用前者。②打需要认证的实例（3080/3098 直接 GET 是 401）时 `?token=` URL 会 303 到 `/`，而 `cdp-probe.mjs` 的 `waitForPageLoad` 要求 `href.startsWith(config.url)` → 报「page load timed out」；拿启动日志里的 token URL 用 curl 换 `dsh-auth-*` cookie，传 `DSH_PROBE_COOKIE=name=value`（探针走 `Network.setCookie`）即可，`.local-tests/grab-cookie.mjs` 是本机做法。
...
- **0.1.5 官方窄屏 expanded 已是原生 overlay 抽屉，插件视觉管理必须让位（2026-09-14 实锤）**：官方 `pI_x6G_sidebarCol` 计算样式 `position:absolute; z-index:1100`（collapsed=52px 官方 rail `pe:none` / expanded=321px `pe:auto`）、frame grid 单轨 390px、自带拖宽把手——官方自己修好了抽屉负优化。插件旧 col 规则的 `z-index:40!important` 会把官方 1100 压到 40，抽屉随即**有 box、computed 正常，却不绘制不命中**（描边实验可证；z 提到 999 仍不画），叠加插件全屏 backdrop（官方原生 expanded 无全屏遮罩）＝「全黑 + 点哪都关」。让位机制：frame-marker 任务每 flush 跑 `updateNativeDrawerGen()`，命中在 `<html>` 挂 `data-mobile-nav-gen="native-drawer"`；layout.css 的 col 规则/开态 transform/reduced-motion/drag-handles 用 `:root:not([data-mobile-nav-gen="native-drawer"])` 门控，overlay-backdrop-fab 该代不建 backdrop（FAB 留 hero）。**检测必须用结构类名（col 类含 `sidebarCol`）＋ computed position，不能用 computed z-index**——检测跑在我们 CSS 在场时，读 z 会读到自压的 40 永久 false（死锁实锤）。手势层零改动兼容（commit 本来就是 `toggleSidebar`）；`cdp-swipe-failures` 12 场景全过。官方 expanded 无遮罩＝内容可点语义，抽屉外点击走官方收起按钮，与 backdrop「点遮罩关」语义不同属预期。升级对账点：`sidebarCol` 类子串（0.1.5 哈希 `pI_x6G_` 会变）。完整取证 → `docs/maintenance/pitfalls.md` §0.1.5 让位。
...
- Automated gates: `pnpm verify` (typecheck) and `pnpm test:core`（13 个测试文件，glob 覆盖 `tests/` 全部）. `pnpm build` additionally exercises the custom client bundler. Use `git diff --check` for whitespace hygiene.
...
- **Playwright MCP 可用（本机已装，2026-09-14 实测），设备仿真验证优先走 MCP；仓库内脚本化回归仍走原生 CDP**：MCP 自带 Chromium（`~/.cache/ms-playwright/chromium-1232`，149.0.7827.155）在本机正常起浏览器并访问 127.0.0.1 的 DSH Web；`browser_run_code_unsafe` 里 `browser.newContext({ viewport: { width: 390, height: 844 }, isMobile: true, hasTouch: true })` + 从已有 context 复制 cookie + `addInitScript` 写 `localStorage['dsh.sessions.current']` 就是一台手机，被桌面布局隐藏的元素用 `document.querySelector(sel).click()` 而非 `page.click()`（后者要可见性检查，会失败）。已用它独立复跑 Files 面板 safe-area 几何断言 13/13 过。两个环境前提：`~/tmp/pw-dsh-tmp` 必须先存在（否则报 `mkdtemp ENOENT`），该 Chromium 协议里没有 `Emulation.setSafeAreaInsets`（回 was not found），inset 仍只能模拟。仓库内 `playwright-core` 的 registry 在 android 平台依旧抛 `Unsupported platform: android`，脚本化回归用原生 CDP，可行做法：spawn 系统 chromium（`--headless=new --no-sandbox --disable-dev-shm-usage --remote-debugging-port=<port> --user-data-dir=<dir>`）+ fetch `/json` 取 webSocketDebuggerUrl + 原生 WebSocket 收发 CDP（Page.navigate / Runtime.evaluate(returnByValue) / Input.dispatchMouseEvent / Page.captureScreenshot / Emulation.setDeviceMetricsOverride）；可参考 `scripts/cdp-probe.mjs` 的 createCdpClient 实现。会话注入仍在导航前 `Page.addScriptToEvaluateOnNewDocument` 写 `localStorage['dsh.sessions.current']`。
...
- 回归探针：`scripts/probes/`（12 个回归锚点，node:builtin-only，可单跑；主探针 `pnpm smoke:cdp` 与手势门 `cdp-swipe-failures.mjs` 见 Commands）。
