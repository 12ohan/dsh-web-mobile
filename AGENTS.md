...
- Key layout（注释版仓库树；`(不入库)` = gitignore，外部 clone 不可见）:
  ...
  │     ├─ core/             ← reconciler-core.ts（零 import）+ raf-scheduler.ts · css-rules.ts · sessions-compat.ts · layout-compat.ts
  │     ├─ effects/          ← 15 个效果模块：phone-chrome · sidebar-swipe ·
...
- Shared full-tree reconciler:
  ...
  - Registered tasks: `frame-marker`, `preview-fullscreen-toggle`, `git-chip-reparent`, `settings-toolbar-reparent`, `preview-close-sync`, `sheet-rise-replay`, `stats-line`, `overlay-backdrop-fab`, `panel-back-exit`.
- Effects:
...
- **审查规模与改动成正比（用户要求，2026-09-19）**：PR/改动审查默认**自己一遍过**（CI 状态 + 静态对账 + 抽读核心 diff），不为流程排场派多代理；只有真大范围改动（跨多模块 / 数百行 diff / 触碰手势与宿主契约）才考虑拆专项，且派前先问用户。
...
- Automated gates: `pnpm verify` (typecheck) and `pnpm test:core`（24 个测试文件，glob 覆盖 `tests/` 全部）. `pnpm build` additionally exercises the custom client bundler. Use `git diff --check` for whitespace hygiene.
