...
② **「未布局副本」的状态签名**：computed 读到规则值（28×34）而 `getBoundingClientRect()` 全 0，**只可能是「节点仍连接、但某祖先 computed display:none」**（三态对照实测）：分离节点（`cloneNode` / `remove()` 后）的 computed 是**空串**、根本读不到 28×34；composer stack 上加 `content-visibility:hidden` 则完全不改变 rect。本宿主里能当那个祖先的是 **frame（app 根）内联 `display:none`**，即既有「fence-only」页面状态——把 frame 内联 display:none 即复现同一签名。此时探针的 `rect.width > 0` 过滤给 0 张可用卡（`0.composer-present` 红），而**只看 computed 的旧写法在同一时刻仍报 28×34 → 假绿**。复现率：8 次全新 profile 连跑（recon 原流程 ×3、真会话 ×2、hero、不存在的会话 id、桌面；各 10 s、自 document_start 起 100 ms 采样）**未复现**；应用真实过渡是「无卡 → hero 卡已布局 356×108 → active 卡已布局 356×98」。状态/签名/判定对照表与取样脚本见 recon 文档 §4.3/§5.1。
