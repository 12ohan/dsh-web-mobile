...
@media (max-width: 1023px) and (pointer: coarse) {
  ...
  /* 头部 UI 的按下反馈（店主 2026-09-23 修正："是头部 UI，没有和输入框那几个功能键
     一样的触发反馈"）。取证：头部那四个宿主包（subagent / agent-preset /
     agent-team / jobs）**:active 全为 0**，胶囊只挂 :hover（触摸不可靠）；
     输入区那边我们已经补了 ::before 胶囊，头部这里同样补。
     只上 background-color（不引入 position/伪元素）：头部芯片里挂着宿主的弹层，
     改 position 可能挪动它们的包含块。圆角沿用宿主自己的。 */
  [data-mobile-nav="frame"] [data-phase] header button:active,
  [data-mobile-nav="frame"] [data-phase] header [role="tab"]:active,
  [data-mobile-nav="frame"] [data-phase] header [role="button"]:active {
    background-color: var(--dsw-alias-interactive-bg-hover, rgba(0, 0, 0, .06));
  }
