 ...
 *
 * 修法（2026-09-23 第三轮定稿）：菜单出现时把它**在输入框里水平居中**（菜单中心 = 输入框
 * `[data-composer-card]` 的中心），重写它的 inline `left`（只改 left，`top` 保持宿主算好的值）。
 * 真机：卡片 16..342、菜单宽 246 ⇒ left **56**（即 56..302，左右各留约 40px）。
 * 拿不到卡片时退回「居中于触发器 + 视口 GUTTER」。只认模型菜单的哈希锚点，其它菜单一律不碰。
 *
...
function installModelMenuAnchor(ctx) {
    (0, phone_chrome_ts_1.installMobileEffect)(ctx, 'dsh-web-mobile: model menu anchor', () => {
        ...
        const place = () => {
            ...
            // 店主 2026-09-23 第三轮定稿：**菜单在输入框里水平居中**（菜单中心 = 输入框中心）。
            // 真机：卡片 16..342、菜单宽 246 ⇒ left 56（即 56..302，左右各留约 40px）。
            // 历史：宿主自己「右缘贴触发器右缘」落 12..258（店主第一眼觉得偏左）；
            // 「居中于触发器」落 106..352（触发器在右半边 234 ⇒ 整块看着偏右）。
