...
@media (max-width: 1023px) and (pointer: coarse) {
  ...
  /* --- 0.1.6-alpha.2 session-header adaptation (audited on a real device) ---
     ...
     landing the 14 items whose anchors exist on every host generation. Two preset items
     (#6/#7) are deliberately omitted: they anchor on .dsha-preset-header-anchor, a marker
     that exists only in the DSHA build, so they would be dead rules here. Every rule below is
     keyed on 0.1.6 markup (_headerLeading / _crumbSeg / _headerCorner / _headerUtilities) or on
     shared chip roots, and each carries its measured before/after from the audit. */
  [data-mobile-nav="frame"] [data-phase] header {
